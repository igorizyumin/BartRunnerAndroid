...
- Directory captures inventoried: **11**; supplemental raw/JSON sources inventoried separately: **28**.
- Raw GTFS-Realtime trip updates normalized: **579**; stop-time rows: **6781**.
- ETD predictions normalized: **3586** from **7** directory captures.
- Explicit realtime cancellations: **2** trip updates; static-only snapshot rows are not called cancellations.
- Explicit association states: {'exact': 492, 'operational_telemetry': 43, 'realtime_only': 10, 'required_counterpart_not_observed': 34}; see `gtfsrt_associations.csv`.
...
| `bart_live_20260917_123018` | 96 trips / 1168 stops | 49 | 2 | 2026-09-17T19:30:18+00:00 | none |
...
Active static trips across observed service dates: **4432**. Duplicate trip IDs in `trips.txt`: **0** by ID key (the normalized table preserves service-date rows).
...
Mapped realtime timing observations: **6828**; median deviation from static scheduled time: **73s**; p90 absolute deviation: **497s**. These are operational timing corrections, not evidence that static schedule structure is wrong. See `static_realtime_timing.csv`.
...
| Metric | Count |
|---|---:|
| trip updates | 579 (100.0%) |
| missing route IDs | 579 (100.0%) |
| stop rows missing sequences | 6781 |
| partial stop lists | 492 (85.0%) |
| explicit CANCELED updates | 2 (0.3%) |
| trip IDs absent from static | 87 (15.0%) |
| numeric 600–799 entity IDs | 77 (13.3%) |
| duplicate trip IDs within feed | 20 (3.5%) |
| stale origin predictions | 24 (4.1%) |
| updates ending before static terminal | 491 (84.8%) |
...
The audit found **77** numeric 600–799 records across **8** captures. **77** have no route ID and **0** map directly to static trip IDs. Consecutive-capture identity observations are in `dmu_consecutive.csv`; shared IDs are evidence of persistence only, not passenger identity.
...
Static trips serving both SFO/SFIA and Millbrae: **468**; of these, Red route 7/8 direct rows: **336**, Yellow route 1/2 rows: **132**. GTFS-RT records touching SFO or Millbrae: **68**. ETD rows labeled Millbrae/SFO-Millbrae: **603**.
