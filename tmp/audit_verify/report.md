...
Static feed version: `72`, feed dates `20260112`–`20260830`. Both checked-in ZIPs were parsed; the audit uses the first and records both in `static_sources.json`.
...
The parser treats `24:xx:xx` as seconds after midnight rather than rolling it to the prior date. Service-date activation applies `calendar.txt` first and `calendar_dates.txt` exceptions second. Platform stop IDs normalize through `parent_station`, preserving the raw platform ID in stop-time records.
...
A DMU record without an electric/static counterpart remains operational telemetry. The evidence does not justify inventing a passenger trip identity or treating absence of a DMU row as cancellation.
...
The static feed therefore does contain an explicit Red SFO–Millbrae service pattern, while Yellow also has direct-through patterns. This is source-backed schedule structure, not proof that every late-night operational movement has a corresponding realtime identity. ETD labels are not promoted to trip identity.
...
The audit distinguishes `explicit CANCELED`, `static_only_in_snapshot`, and unmatched ETD/GTFS-RT rows. It does not infer a false-positive or false-negative omission rate from unlabeled snapshots.

## Cross-source matching

ETD matching uses station, color-derived line, platform/direction where available, destination alias, and absolute time within 180 seconds. Greedy ordering is deterministic and each ETD/GTFS-RT row can be used once. `ambiguous` and unmatched rows remain in `matches.csv` with reasons.
